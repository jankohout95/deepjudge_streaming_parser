...
# The following implementation of custom CustomQueue to avoid NotImplementedError
...
class CustomQueue(Queue):
    ...
    serialize CustomQueue when it is passed between processes. If these functions
    are not defined, CustomQueue cannot be serialized, which will lead to the error
    of "AttributeError: 'CustomQueue' object has no attribute 'size'".
    ...
    def __getstate__(self):
        """Help to make CustomQueue instance serializable.
        Note that we record the parent class state, which is the state of the
        actual queue, and the size of the queue, which is the state of CustomQueue.
